...
nmap_default () {
  if [ $# -eq 0 ]
    then
      ...
      sudo nmap -sCV -T4 --min-rate 1000 "${@}" -v -oA nmap/tcp_default -p- 
  fi
}
...
alias script_tty_upgrade="echo '/usr/bin/script -qc /bin/bash /dev/null'"
alias tty_fix="stty raw -echo; fg; reset"
alias tty_conf="stty -a | sed 's/;//g' | head -n 1 | sed 's/.*baud /stty /g;s/line.*//g'"
